...
for dirName in */;do
  ...
  for proto_file in ${dirName}*.proto; do
      ...
      if [ ! -e $pb_file ] || [ $proto_file -nt $pb_file ]; then
          ...
          $PROTOC --proto_path=${dirName} --proto_path=. --go_out=../${pb_package}/${dirName} --go-grpc_out=../${pb_package}/${dirName}  --go_opt=paths=source_relative $proto_file --plugin=protoc-gen-go=../../bin/protoc-gen-go.exe
      fi
  done
done